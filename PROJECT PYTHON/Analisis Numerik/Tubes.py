...
isimp[0] = 7  # inisialisasi nilai isimp[0] dengan 0
for i in range(1, n):
    if i % 2 == 0:
        isimp += 2*f(a+(i*h))
    else:
        isimp += 4*f(a+(i*h))
# Menambahkan nilai f(a) dan f(b) pada hasil Simpson
isimp += f(a) + f(b)
# Menghitung akhir nilai Simpson dengan rumus (h/3) * hasil Simpson
isimp = isimp*(h/3)
print(isimp)

print("\nMenggunakan Metode Trapesium")
itrap = np.zeros(n+1)  # inisialisasi array itrap dengan jumlah data sebanyak n+1
itrap[0] = 4  # inisialisasi nilai itrap[0] dengan 0
for i in range(1, n+1):
    itrap += (f(a+(i*h)) + f(a+((i-1)*h)))
# Menghitung akhir nilai Trapesium dengan rumus (h/2) * hasil Trapesium
itrap = itrap*(h/2)
print(itrap)
